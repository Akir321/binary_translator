...
static const char *NameAddSymbols = ".asm";


static void pointToZero(char *str);

static int checkSignature (FILE *fin);
static int checkComVersion(FILE *fin);

static int printArgument(int command, int argument, FILE *fout);

static int myFputsCommandName(const char *str, FILE *f);


// change -> only some code_gen func call here
// the func call is written in commands.h
// the func is defined in a separate file "code_gen_funcs.cpp"
...
int runDisassembler(int *bufIn, FILE *fout)
{
    ...
    while (*bufIn != HLT) // what if we have multiple hlt's in our program
    {   
        switch (*bufIn)
        {
            #include "commands.h"
    ...
    fprintf(fout, "%s", "hlt"); // same question as up
    return EXIT_SUCCESS;
}
